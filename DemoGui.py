...
menu_icon=cv2.imread('menu.png')
cal_icon=cv2.imread('cal.jpg')
draw_icon=cv2.imread('draw.jpg')
music_icon=cv2.imread('music.jpg')
show_menu=False
...
def menuMode(im,realHandContour,realDefect,nodefect):
    global show_menu
    if (nodefect>0):
        listEndPoint=[]
        ...
        cx =end[0]
        cy =end[1]
        if cy in range(30, 80) and cx in range (30,80):
...
while True:
    ...
    if tracked is False:
        ...
        drawframe[30:80,30:80]=menu_icon
        menuMode(drawframe,realHandContour,realDefect,noDefect)
        if show_menu is True:
            drawframe[110:160, 30:80] = cal_icon
            drawframe[180:230, 30:80] = draw_icon
            drawframe[250:300, 30:80] = music_icon


        #cv2.rectangle(drawframe, (0, 220), (250, 320), (255, 0, 0), 1)
